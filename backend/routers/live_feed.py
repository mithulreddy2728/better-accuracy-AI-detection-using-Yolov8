...
def load_models():
    ...
    if MODELS["reader"] is None:
        ...
        try:
            ...
            MODELS["reader"] = PaddleOCR(use_angle_cls=True, lang='en', use_gpu=False, show_log=False)
            print("✓ PaddleOCR initialized successfully for live feed")
        except Exception as paddle_err:
            print(f"⚠ PaddleOCR not available for live feed, falling back to Tesseract: {paddle_err}")
        ...
        if MODELS["reader"] is None:
            try:
                ...
                MODELS["reader"] = TesseractPaddleWrapper(pytesseract)
                print(f"✓ Tesseract OCR initialized as live feed fallback (version: {test_version})")
            except Exception as e:
...
def generate_frames(camera_id: int, camera_source: str, db: Session):
    ...
    # Check if RTSP/HTTP network stream or local file
    is_network_stream = any(camera_source.lower().startswith(p) for p in ['rtsp://', 'rtsps://', 'http://', 'https://'])
    if not is_network_stream and not os.path.isabs(camera_source):
