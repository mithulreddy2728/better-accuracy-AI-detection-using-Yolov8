...
const CameraOutput = () => {
  ...
  const fetchCameras = async () => {
    try {
      ...
      const response = await axios.get(getApiUrl("/cameras/list"), {
  ...
  const fetchVehicles = async (cameraId) => {
    try {
      ...
      const response = await axios.get(
        getApiUrl(`/vehicles/by-camera/${cameraId}`),
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const fetchGeoMarkers = async (cameraId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        getApiUrl(`/geo-markers/by-camera/${cameraId}`),
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const loadVideo = (camera) => {
    if (videoRef.current) {
      if (camera.type === 1) {
        ...
        videoRef.current.src = getApiUrl(`/media/${camera.source}`);
      }
    }
  };
